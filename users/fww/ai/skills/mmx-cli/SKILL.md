...
```bash
# Install — nix 声明式安装(pkgs.mmx-cli,见 nixos-config/pkgs/mmx-cli),勿用 npm
...
CLI flags → `~/.mmx/config.json` → defaults.

> **Note (verified against the v1.0.19 binary):** the `MINIMAX_API_KEY`
> environment variable is **NOT** part of credential resolution — it only
> triggers an interactive "save to config?" prompt and is silently skipped
> under `--non-interactive`. For non-interactive use, pass `--api-key`,
> or point `MMX_CONFIG_DIR` at a directory containing a `config.json`.
> The nix-packaged `mmx` wrapper auto-injects `--api-key` from
> `/run/secrets/minimax_api_key` when no explicit `--api-key` is given.
...
# Environment (region IS honored; the key env is NOT — see note above)
