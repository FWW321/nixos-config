...
<!-- Vendored from https://github.com/michalzubkowicz/nixos-management-skill nixos-managing/ (2026-08-19, 内容快照 2026-07 对齐 NixOS 26.05)
     本地适配:
     - MCP 段"ask first"改为直接可用(本环境 mcp-nixos v3 常驻于 mcp.nix,无需逐任务询问)
     - 工具接口与 v3 一致(nix/nix_versions),未改动
     与 nix/nixpkgs skill 分工:本条 = 系统运维(rebuild/装机/加密/监控),nix = 判断/约束层,nixpkgs = 打包参考 -->
...
If the `mcp-nixos` MCP server ([utensils/mcp-nixos](https://github.com/utensils/mcp-nixos)) is configured in the environment, prefer it for lookups over SSH/web round-trips. In this environment it is always configured — use it directly without asking.
