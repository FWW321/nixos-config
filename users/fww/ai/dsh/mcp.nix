...
let
  ...
  header =
    ...
    if builtins.isString v then
      ...
      { inherit (v) secretFile; } // lib.optionalAttrs (v ? prefix) { inherit (v) prefix; };
  ...
  skillSource =
    ...
    in
    if builtins.pathExists "${src}/SKILL.md" then
      s.source
    else if builtins.pathExists "${src}/skills/${name}/SKILL.md" then
      "${src}/skills/${name}"
    else
      s.source;
in
{
      ...
      (
        ...
        if s ? remote then
        ...
        else
          {
            ...
            env = lib.mapAttrs (_: v: if v ? secretFile then { inherit (v) secretFile; } else v) (
              s.local.env or { }
            );
          }
      )
  ...
  programs.dsh.skills = lib.mapAttrs (name: s: { source = skillSource name s; }) (
