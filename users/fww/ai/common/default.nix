...
let
  ...
  skillsViolations = lib.concatLists (lib.mapAttrsToList
    (name: s:
      if !(s ? source) then
        [ "skill \"${name}\": 缺 source 字段" ]
      else if s ? entryFile then
        lib.optional (!(builtins.pathExists "${s.source}/${s.entryFile}"))
          "skill \"${name}\": ${s.source}/${s.entryFile} 不存在(上游改名/移动?)"
      else
        lib.optional (!(builtins.pathExists "${s.source}/SKILL.md"))
