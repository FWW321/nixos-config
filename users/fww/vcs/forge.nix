...
# 系统级 gh hosts.yml 模板(modules/system/secrets.nix)也从 forges.nix 取 GitHub 用户名
...
in
{
  ...
  # 认证由系统 sops 模板渲染 ~/.config/gh/hosts.yml(见 modules/system/secrets.nix),无需 gh auth login
  programs.gh = {
    enable = true;
    settings = {
