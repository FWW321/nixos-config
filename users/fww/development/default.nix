...
#   ✅ rust.nix     = Rust 语言生态（rust-overlay 只是该域的工具提供者）
...
#
# ── 本文件（default.nix）──
# 纯聚合入口：只 imports 各语义域文件 + 阐明分类标准。
# 工具一律归其所属语言域（含 gcc → c-cpp.nix）。
# 例外：devenv 是跨域元工具（管 per-repo 环境，不属任何语言生态），无对应语义域文件，故直接在此安装。
